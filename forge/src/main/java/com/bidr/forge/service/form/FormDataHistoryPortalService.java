...
public class FormDataHistoryPortalService extends BasePortalService<FormDataHistory, FormDataHistoryVO> {
    ...
    public void submit(String historyId) {
        ...
        Validator.assertTrue(ApprovalDict.UNKNOWN.getValue().equals(history.getStatus())
                        || (ApprovalDict.REJECT.getValue().equals(history.getStatus())),
                ErrCodeSys.SYS_ERR_MSG,
                DictEnumUtil.getEnumByValue(history.getStatus(), ApprovalDict.class, ApprovalDict.UNKNOWN).getLabel() + "记录不能提交");
        history.setStatus(ApprovalDict.APPLY.getValue());
