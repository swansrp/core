...
import com.bidr.insight.flow.ChatBiFlowConversationListener;
import com.bidr.insight.service.ChatBiConversationService;
import com.bidr.insight.service.ChatBiSensitiveService;
import com.bidr.insight.sse.ChartSpecExtractor;
import com.bidr.insight.sse.ChartSpecSensitiveGuard;
    ...
    private void extractChartSpec(String fullContent, FlowContext context) {
        ...
        if (specJson != null) {
            ...
            context.setVariable(FlowContext.TRACE_EXTRACT_RESULT, "chartSpec 提取成功（" + specJson.length()
                    + " 字" + (stripped ? "，敏感列引用已剔除" : "") + "）");
            sender.send(FlowSseSender.EVENT_SPEC, specJson);
            chartSpec = specJson;
        } else {
        ...
        sender.send(FlowSseSender.EVENT_DONE, answer);
        sender.complete();
    }
}
