...
import com.bidr.insight.chatbi.flow.ChatBiAskFlowDefinition;
import com.bidr.insight.chatbi.vo.ChatBiAskReq;
import com.bidr.llm.flow.AbstractFlowAskAgent;
...
/**
 * Title: ChatBiService
 * Description: 智能问数编排入口——继承 {@link AbstractFlowAskAgent} 模板（建连/组装/执行/
 * 失败收口样板已固化），只填业务钩子：变量注入 + 对话落库副作用（fillContext）、
 * 发起失败补写错误回复（onStartFailed），执行 ask 链
...
@Slf4j
@Service
public class ChatBiService extends AbstractFlowAskAgent<ChatBiAskReq> {
    ...
    public ChatBiService(FlowEngine flowEngine,
                         ChatBiConversationService chatBiConversationService,
                         ChatBiSensitiveService chatBiSensitiveService) {
        super(flowEngine);
        this.chatBiConversationService = chatBiConversationService;
        this.chatBiSensitiveService = chatBiSensitiveService;
    }

    @Override
    protected String flowKey() {
        return ChatBiAskFlowDefinition.FLOW_KEY;
    }

    /**
     * 访问人（人名优先回落工号，轨迹/对话按人隔离记录）：
     * SSE 挂起后回调线程读不到 AccountContext，模板在发起线程先取
     */
    @Override
    ...
    /**
     * 业务变量注入 + 前置副作用：提问即写历史对话（新对话在此创建，id 经 conv 事件回传），
     * 助手回复由引擎链路收口时补写
     */
    @Override
    protected void fillContext(FlowContext context, SseEventSender sender, ChatBiAskReq req) {
        ...
        String conversationId = chatBiConversationService.appendUser(
                req.getConversationId(), context.getOperator(), req.getQuestion(), req.getTableId(), req.getPortalName());
        context.setConversationId(conversationId);
        // conv 事件先于 delta 下发：前端拿到对话 id 后续提问续接同一对话
        sender.send(SseEventSender.EVENT_CONV, conversationId);
    }

    /**
     * 发起即失败收口：对话已创建则补写一条错误回复，避免停在只有提问的半轮；msgid 先于 error 下发供评价定位
     */
    @Override
    protected void onStartFailed(FlowContext context, SseEventSender sender, ChatBiAskReq req, Exception error) {
        String conversationId = context.getConversationId();
        if (conversationId == null) {
            return;
        }
        log.warn("智能问数发起失败, tableId={}", req.getTableId(), error);
        String messageId = chatBiConversationService.appendAssistant(conversationId, failureMessage(error), null,
    ...
    protected String failureMessage(Exception error) {
        return error.getMessage() == null ? "智能问数发起失败" : error.getMessage();
    }
