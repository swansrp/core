...
public class KafkaProducerService {
    ...
    private KafkaTemplate<String, String> resolveTemplate() {
        if (transactionEnabled && transactionalKafkaTemplate != null && TransactionSynchronizationManager.isActualTransactionActive()) {
    ...
    public CompletableFuture<SendResult<String, String>> sendAsync(String topic, String message) {
        ...
        ListenableFuture<SendResult<String, String>> listenableFuture = resolveTemplate().send(topic, message);
    ...
    public CompletableFuture<SendResult<String, String>> sendAsync(String topic, String key, String message) {
        CompletableFuture<SendResult<String, String>> future = new CompletableFuture<>();
        ListenableFuture<SendResult<String, String>> listenableFuture = resolveTemplate().send(topic, key, message);
    ...
    public CompletableFuture<Void> sendBatch(String topic, Iterable<String> messages) {
        ...
        // 无事务时：异步发送，提升吞吐量
        CompletableFuture<Void> allFutures = CompletableFuture.allOf(java.util.stream.StreamSupport.stream(messages.spliterator(), false).map(message -> sendAsync(topic, message)).toArray(CompletableFuture[]::new));
        return allFutures;
    }
