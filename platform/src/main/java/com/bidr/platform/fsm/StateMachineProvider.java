...
public class StateMachineProvider implements ApplicationContextAware {
    ...
    public static MachineTransition doTransfer(MachineType machineType, String stateFromValue, Class machineStateClazz,
    ...
    public static MachineTransition doTransfer(MachineType machineType, MachineState from, MachineOperate operate,
                                               List<?> roleList) {
        return doTransfer(machineType, from, operate, roleList, ErrCodeSys.STATE_MACHINE_TRANSFER_NOT_ALLOW,
                from.getLabel(), operate.getLabel());
    }

    public static MachineTransition doTransfer(MachineType machineType, MachineState from, MachineOperate operate,
                                               List<?> roleList, ErrCode tipCode, String... tipMsg) {
        StateMachine stateMachine = STATE_MACHINE_MAP.get(machineType.getValue());
        ...
        return STATE_MACHINE_MAP.get(machineType.getValue()).doTransfer(from, operate);
    }

    public static void validateMachineRoleOperate(MachineType machineType, MachineOperate operate, List<?> roleIdList) {
        StateMachine stateMachine = STATE_MACHINE_MAP.get(machineType.getValue());
        validateMachineRoleOperate(stateMachine, operate, roleIdList);
    }

    public static void validateMachineRoleOperate(StateMachine stateMachine, MachineOperate operate,
                                                  List<?> roleIdList) {
    ...
    public static MachineTransition doTransfer(MachineType machineType, MachineState from, MachineOperate operate) {
        return doTransfer(machineType, from, operate, null, ErrCodeSys.STATE_MACHINE_TRANSFER_NOT_ALLOW,
                from.getLabel(), operate.getLabel());
    }

    @SuppressWarnings("rawtypes, unchecked")
    public static MachineTransition doTransfer(MachineType machineType, String stateFromValue, Class machineStateClazz,
                                               MachineOperate operate, List<String> roleList) {
        Enum machineState = DictEnumUtil.getEnumByValue(stateFromValue, machineStateClazz);
        return doTransfer(machineType, (MachineState) machineState, operate, roleList);
    }

    public static MachineTransition doTransfer(MachineType machineType, MachineState from, MachineOperate operate,
                                               ErrCode tipCode, String... tipMsg) {
        return doTransfer(machineType, from, operate, null, tipCode, tipMsg);
    }

    public static void validateMachineRoleOperate(MachineType machineType, MachineOperate operate, String role) {
        StateMachine stateMachine = STATE_MACHINE_MAP.get(machineType.getValue());
        validateMachineRoleOperate(stateMachine, operate, role);
    }

    public static void validateMachineRoleOperate(StateMachine stateMachine, MachineOperate operate, String role) {
        Set<MachineRole> acceptRoleSet = stateMachine.getAcceptRoles(operate);
        if (CollectionUtils.isNotEmpty(acceptRoleSet)) {
            boolean allow = false;
     ...
     * @return 状态机
     */
    public StateMachine getStateMachineByType(String type) {
        StateMachine stateMachineDef = STATE_MACHINE_MAP.get(type);
        Validator.assertNotNull(stateMachineDef, ErrCodeSys.SYS_CONFIG_NOT_EXIST, type);
        return stateMachineDef;
    }

    @Override
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        for (Map.Entry<String, StateMachine> entry : applicationContext.getBeansOfType(StateMachine.class).entrySet()) {
    ...
    private void registerStateMachine(StateMachine stateMachine) {
        STATE_MACHINE_MAP.put(stateMachine.getMachineType().getValue(), stateMachine);
        stateMachine.init();
    }
}
