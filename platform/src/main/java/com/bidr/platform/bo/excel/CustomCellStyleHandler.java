...
/**
 * Title: CustomCellStyleHandler
 * Description: Copyright: Copyright (c) 2023
 *
 * @author Sharp
 * @since 2024/02/19 10:04
 */

/**
 * 自定义单元格样式处理器（支持字体样式、背景颜色、边框样式、对齐方式、自动换行）
 */
public class CustomCellStyleHandler implements RowWriteHandler {
    /**
     * sheet页名称列表
     */
    ...
    public void afterRowDispose(WriteSheetHolder writeSheetHolder, WriteTableHolder writeTableHolder, Row row
        ...
        if (cellStyleList == null || cellStyleList.size() <= 0 ||
                !sheetNameList.contains(sheet.getSheetName())) {
            return;
        }
        //获取当前行的样式信息
        List<CellStyleModel> rowCellStyleList = cellStyleList.stream().filter(x ->
                        StrUtil.equals(x.getSheetName(), sheet.getSheetName()) && x.getRowIndex() == relativeRowIndex)
                .collect(Collectors.toList());
        //该行不需要设置样式
