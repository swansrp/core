 ...
 *
 * @author Sharp
 * @since 2025/6/12 15:37
 */

public class MarkdownUtil {

    public static <T> String mapToTable(Map<?, T> map, Class<T> clazz) {
        return listToTable(map.values(), clazz);
    }

    public static <T> String listToTable(Collection<T> list, Class<T> clazz) {
        Map<String, String> fieldDisplayMap = ReflectionUtil.getFieldDisplayMap(clazz);
        String[] titles = fieldDisplayMap.values().toArray(new String[0]);
        List<String[]> data = new ArrayList<>();
        if (FuncUtil.isNotEmpty(list)) {
            for (T t : list) {
                List<String> array = new ArrayList<>();
                for (String s : fieldDisplayMap.keySet()) {
                    array.add(JsonUtil.toJson(ReflectionUtil.getFieldValue(t, s), false, false, true));
                }
                data.add(array.toArray(new String[0]));
            }
        }
        return MarkdownUtil.of().table().data(titles, data.toArray(new String[0][0])).endTable().build();
    }
    ...
    public static class SectionBuilder {
        ...
        public SectionBuilder text(String text, Style... style) {
            if (text != null) {
                curSec.addChild(new Section(Section.Type.NORMAL, new MetaData(Fonts.of(text, style)), curSec, null,
                        curSec.getDepth()));
            }
            return this;
        }
        ...
        public SectionBuilder text(String name, String value) {
            if (name != null) {
                ...
                curSec.addChild(new Section(Section.Type.NORMAL,
                        new MetaData(MetaData.Type.NORMAL, Fonts.of(name, (Style) null), values), curSec, null,
                        curSec.getDepth()));
            }
            return this;
        }
        ...
        public SectionBuilder bigTitle(String title) {
            if (FuncUtil.isNotEmpty(title)) {
                curSec.addChild(new Section(Section.Type.BIG_TITLE, new MetaData(Fonts.of(title)), curSec, null,
                        curSec.getDepth()));
            }
            return this;
        }
        ...
        public SectionBuilder title(String title, Style color) {
            if (FuncUtil.isNotEmpty(title)) {
                curSec.addChild(new Section(Section.Type.TITLE, new MetaData(Fonts.of(title, color)), curSec, null,
                        curSec.getDepth()));
            }
            return this;
        }
        ...
        public SectionBuilder title(String title, Style titleColor, Fonts... label) {
            if (FuncUtil.isNotEmpty(title)) {
                ...
                curSec.addChild(
                        new Section(Section.Type.TITLE, new MetaData(Fonts.of(title, titleColor), labelList), curSec,
        ...
        public SectionBuilder subTitle(String title) {
            if (FuncUtil.isNotEmpty(title)) {
                curSec.addChild(new Section(Section.Type.SUBTITLE, new MetaData(Fonts.of(title)), curSec, null,
                        curSec.getDepth()));
            }
            return this;
        }
        ...
        public SectionBuilder links(String name, Map<String, String> urlMappings) {
            if (urlMappings != null && !urlMappings.isEmpty()) {
                ...
                Fonts wrappedName = FuncUtil.isNotEmpty(name) ? Fonts.of(name) : Fonts.EMPTY;
        ...
        public SectionBuilder ol() {
            int depth = (curSec.getType() == Section.Type.ORDER_LIST ||
                    curSec.getType() == Section.Type.UN_ORDER_LIST) ? curSec.getDepth() + 1 : curSec.getDepth();
        ...
        public SectionBuilder br() {
            curSec.addChild(
                    new Section(Section.Type.BR, new MetaData(MetaData.Type.BR), parentSec, null, curSec.getDepth()));
            return this;
        }
