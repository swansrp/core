...
4. **消息通道纯轮询**（会话事件流/状态/控制指令走 Redis，2s 增量拉取，分布式多实例天然兼容）；
   ...
   **例外（2026-09-24 补）**：当长链路的**耐久与续传由中转层承担**时（上游事件带持久序号 +
   快照帧 + 显式挂流重挂，状态不落某实例内存），SSE 同样满足"跨请求拼接、多实例、断线恢复"
   三项要求，可走 SSE 档。判据不是链路长短，而是**耐久缓冲放在哪里**：放 Redis 则轮询解耦实例，
   放上游/中转层则 SSE + 显式 resume 成立。两条腿的视图模型与渲染层与传输方式无关。
